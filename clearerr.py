...
#     metadata = t.get_metadata(raw_m['rating_key'])
    print(json.dumps(t.get_path(raw_m['rating_key']), indent=4))
...
for show in t.get_library(tlibraries['TV Shows']):
    print(json.dumps(t.get_path(show['rating_key']), indent=4))
    for season in t.get_children_metadata(show['rating_key']):
        # print(json.dumps(season, indent=4))
        print(json.dumps(t.get_path(season['rating_key']), indent=4))
